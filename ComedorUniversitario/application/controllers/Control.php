...
class Control extends CI_Controller
{
     ...
     public function index()
     {
          ...
          $data['control'] = $this->model_control->ListarControl();         
          $this->load->view('view_control', $data);
	...
	public function guardarControl()
	{
		...
		// echo "entro".$Control->tarjeta;
		$ControlInsertar = $this->input->post();
		// echo "datos desde otra function->".implode(",",$ControlInsertar) ;
   	    $this->model_control->SaveControl($ControlInsertar);
   	    // redirect("control?save=true");
	}
     ...
     public function nuevo()
     {
		...
		$this->ValidaCampos();
		$prueba = $this->input->post();
		// echo "datos->".implode(",",$prueba) ;
		...
		if($this->form_validation->run() == TRUE)
		{
			...
			if($VerifyExist>0)
			{
	            ...
	            $this->load->view('header');
				$this->load->view('view_nuevo_control');
				$this->load->view('footer');
	        }
		...
		else
		{
			...
			$this->load->view('header');
			$this->load->view('view_nuevo_control',$data);
			$this->load->view('footer');
		} 
     }
	 function ValidaCampos()
	 {
		/*Campos para validar que no esten vacio los campos*/
		  $this->form_validation->set_rules("id_comesales", "Comesales", "trim|required");
		 // $this->form_validation->set_rules("apellidos", "Apellidos", "trim|required");
		 $this->form_validation->set_rules("targeta", "Targeta", "trim|required");
		  $this->form_validation->set_rules("fecha", "Fecha", "trim|required");
		  $this->form_validation->set_rules("controldia", "Controldia", "trim|required");
	 ...
	 public function editar($id = NULL){
		...
		if ($this->input->post()) {
			...
			if ($this->form_validation->run() == TRUE){
				...
				$id_insertado = $this->model_control->edit($datos_update,$id);
				redirect('control?update=true');
			...
			$data['datos_control'] = $this->model_control->BuscarID($id);
			if (empty($data['datos_control'])){
				$data['Modulo']  = "Control";
				$data['Error']   = "Error: El id <strong>".$id."</strong> No es Valido, Verifica tu Busqueda !!!!!!!";
				$this->load->view('header');
				$this->load->view('view_errors',$data);
